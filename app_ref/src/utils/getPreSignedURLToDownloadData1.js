...
export const getPreSignedURLToDownloadData1 = async (tokenPayload) => {
  const Token = await getAuthToken();
  console.log("Token:-->", Token);
  console.log("entered in the DownloadData endpoint");
  const token = await generateToken(tokenPayload, Token);
  const baseURL = process.env.REACT_APP_API_BASE_URL;

  try {
    const response = await axios.post(
      `${baseURL}/generateAndDownloadCSV1?t=${Date.now()}`,
      {
        downloadToken: token, // Token in the body
      },
      {
        headers: {
...
export const getPreSignedURLToDownloadParquetData = async (tokenPayload) => {
  const Token = await getAuthToken();
  console.log("Token:-->", Token);
  console.log("entered in the DownloadData endpoint");
  const token = await generateToken(tokenPayload, Token);
  ...
  try {
    const response = await axios.post(
      `${baseURL}/generateAndDownloadParquetCSV?t=${Date.now()}`,
      {
        downloadToken: token, // Token in the body
      },
      {
