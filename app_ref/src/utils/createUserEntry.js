...
import { getAuthToken, refreshSessionToken } from "../redux/actions/authActions";
...
export const addUserToDatabase = async (token, authToken, userID) => {
  console.log("entered in the add user");
  const baseURL = process.env.REACT_APP_API_BASE_URL;
  const Token = await getAuthToken(userID); 

  try {
    const response = await axios.post(
      `${baseURL}/user?t=${Date.now()}`,
      {
        userDataToken: token, // Token in the body
      },
      {
        headers: {
...
// Function to make a PUT request to update a user in the database
export const updateUserInDatabase = async (userID, userName, userTableName, invitationTableName) => {
  console.log("entered in the update user");
  const baseURL = process.env.REACT_APP_API_BASE_URL;
  ...
  try {
    const response = await axios.put(
      `${baseURL}/user?t=${Date.now()}`,
      {
        userID, // User ID in the body
        userName, // User Name in the body
        userTableName, // User Table Name in the body
        invitationTableName
      },
      {
        headers: {
