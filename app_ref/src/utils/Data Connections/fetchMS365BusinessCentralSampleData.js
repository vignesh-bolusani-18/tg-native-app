...
export const fetchMS365BusinessCentralSampleData = async (tokenPayload, userID) => {
  const Token = await getAuthToken(userID);
  console.log("Token:-->", Token);
  console.log("entered in the fetch MS365BusinessCentral sample data");
  const token = await generateToken(tokenPayload, Token);
  const baseURL = process.env.REACT_APP_API_BASE_URL;

  try {
    const response = await axios.post(
      `${baseURL}/ms365BusinessCentralConnections?t=${Date.now()}`,
      {
