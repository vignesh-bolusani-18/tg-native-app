...
export const fetchTGInternalSampleData = async (tokenPayload, userID) => {
  const Token = await getAuthToken(userID);
  console.log("Token:-->", Token);
  console.log("entered in the fetch Internal Sample Data");
  const token = await generateToken(tokenPayload, Token);
  const baseURL = process.env.REACT_APP_API_BASE_URL;

  try {
    const response = await axios.post(
      `${baseURL}/fetchInternalSample?t=${Date.now()}`,
      {
