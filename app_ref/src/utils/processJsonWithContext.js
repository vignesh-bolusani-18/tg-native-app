...
export const processJsonWithContext = (jsonObject, context) => {
  ...
  const evaluateScript = (scriptContent) => {
    try {
      ...
      console.log("Script body:", scriptBody);
      // Create parameter names from context keys
      const paramNames = Object.keys(context);
      console.log("Param names:", paramNames);
      console.log("Context:", context);
      try {
        // Check if all variables used in the script are available in context

        // Create a function with dynamic parameters based on context
        let scriptFn;
        try {
          scriptFn = new Function(...paramNames, scriptBody);
        } catch (err) {
          console.error("Error creating script function:", err);
          return scriptContent;
        }
        console.log("Script function:", scriptFn);

        // Execute the script with all context values
        const result = scriptFn(...Object.values(context));

        console.log("Result:", result);
        //console.log("Result:", result);

        // If result is undefined or null, return the original script
      ...
      // Handle any errors during script parsing or function creation
    ...
    if (jsonObject.every((item) => typeof item === "string")) {
      ...
      for (const item of jsonObject) {
        ...
        if (item.startsWith("Script(")) {
          ...
          Object.keys(context).some((key) => item.startsWith(`${key}.`))
        ) {
    ...
    for (const [key, value] of Object.entries(jsonObject)) {
      if (typeof value === "string") {
        if (value.startsWith("Script(")) {
          ...
          console.log("Processing script expression:", value);
...
const demonstrateCapabilities = () => {
  ...
  const demoObject = {
    ...
    nestedArrays: {
      columns:
        "Script(const cols = config.etl.loaded_datasets.inventory[0].field_tags.replenishment_columns; return [...cols, 'additional_col'];)",
    },
    // Test combining multiple context references
    combinedReferences: {
      combinedColumns:
        "Script(const inventoryCol = config.data.inventory_column; const replenishmentCols = config.etl.loaded_datasets.inventory[0].field_tags.replenishment_columns; return [inventoryCol, ...replenishmentCols];)",
    },
  };
