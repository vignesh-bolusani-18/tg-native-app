import { tagFieldConfigs } from "./tagFieldConfigs";
...
export const setDatasets = (moduleName, state) => {
  switch (moduleName) {
    ...
    case "replenishment":
      ...
      state.optional_datasets_tags = ["item_master", "others", "inventoryothers"];
      break;
...
//  //get dataset render object from the tag
export const getDatasetObject = (tag) => {
  ...
  switch (tag) {
    ...
    case "simple_disaggregation_mapping":
      datasetObject.title = "Disaggregate Mapping";
      datasetObject.description =
        "Additional Information about Disaggregate Mapping";
      break;
    case "rewrite_forecast":
      datasetObject.title = "Forecast Rewrites";
      datasetObject.description =
        "Additional Information about Forecast Rewrites";
      break;
    case "transition_item":
      datasetObject.title = "Transition Item";
      datasetObject.description =
        "Additional Information about Transition Item";
      break;
    default:
...
export const getTagFieldConfig = (tag) => {
  return tagFieldConfigs[tag];
};
...
export const getColumnInUse = (loadedDataset) => {
  // Helper function to get value from loadedDataset by path

  // Gather all tags from tagFieldConfigs
  let allTags = [];
  Object.values(tagFieldConfigs).forEach((tagConfig) => {
    allTags = [
      ...allTags,
      ...tagConfig.mandatory_tags,
      ...tagConfig.optional_tags,
    ];
  });
...
export const areMandatoryFieldsFilled = (tag, loadedDataset) => {
  const mandatoryTags = tagFieldConfigs[tag].mandatory_tags;
  let allFilled = true;
...
export const updateDataBlock = (state, loadedDatasets) => {
  ...
  const getValue = (key, ogValue) => {
    let values = [];
    // Check if all datasets have no that key, then return ogValue
    let allMissing = true;
    for (const tag in loadedDatasets) {
      if (
        loadedDatasets[tag].some(
          (dataset) => dataset.field_tags[key] !== undefined
        )
      ) {
        allMissing = false;
        break;
    ...
    if (isList) {
      ...
      return values.length > 0 ? values[0] : ogValue === "None" ? "None" : null;
    }
  };
  ...
  for (const key in newState.data) {
    if (!keysToSkip.includes(key)) {
      const value = getValue(key, newState.data[key]);
      // if (value !== null) {
  ...
  if (
    ...
    const dimensionValues = getValue(
      dimensionKey,
      newState.scenario_plan.post_model_demand_pattern[dimensionKey]
    );
    if (dimensionValues) {
