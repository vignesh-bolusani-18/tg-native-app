...
export default function ForecastDataReferenceTable({
  ...
  if (!forecastDataReferences || forecastDataReferences.length === 0) {
    return null;
  }
    ...
    <Box sx={{ width: "100%", mt: 2 }}>
      ...
      >
        ...
        >
          ...
          <TableBody>
            {forecastDataReferences.map((reference, index) => {
              const experimentId = getExperimentIdFromPath(reference.exp_path);
              const experimentName = getExperimentNameById(experimentId);
              const moduleName = getExperimentModuleById(experimentId);

              return (
                <StyledTableRow key={`${reference.exp_path}-${index}`}>
                  <StyledTableCell
                    component="th"
                    scope="row"
                    sx={{ width: "40%" }}
