...
const CustomCounter = ({
  ...
  value,
  setValue,
  isTGScenario = null //[{path:string,target:string},...]
}) => {
  const {
  ...
  useEffect(() => {
    if(!path){setInputValue(value);}
    
  }, [value]);


  useEffect(() => {
    setPrecision(getPrecision(defaultValue)); // Update precision when defaultValue changes
  }, [defaultValue]);
  ...
  const handleIncrement = () => {
    ...
    const newValue = parseFloat(((!path? value :val) + incrementValue).toFixed(precision));
  ...
  const handleDecrement = () => {
    ...
    const newValue = parseFloat(((!path? value :val)  - decrementValue).toFixed(precision));
    console.log(
