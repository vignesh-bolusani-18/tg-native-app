...
import NewTagFieldReview from "../pages/main/ExperimentFlow/CreateConfigurableExperimentPage/AddData/NewTagFieldReview";
...
const Row = ({ row, index, onDeleteConfirm }) => {
  // console.log("row", row);
  ...
  const { loadDatasetCSV } = useModule();
  const {
  ...
  const { loadMetaDataFromS3, SetPath, currentDatasetTag, onClickDatasetCard } =
    useModule();
  //temparory
  ...
  return (
    <>
      ...
      />
      <NewTagFieldReview handleClose={handleNewTagsClose} open={newTagsopen} />
    </>
  );
};
