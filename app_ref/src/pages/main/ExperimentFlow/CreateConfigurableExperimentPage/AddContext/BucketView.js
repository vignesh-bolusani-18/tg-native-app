...
const NewBoxComponent = ({
  ...
  const { contextConfig, contextBuckets } = useModule();
        ...
        <Stack p={2}>
          ...
          {missingFeatures.map((feature) => (
            ...
            >
              ...
              >
                Missing required fields: {feature.label}
              </Typography>
            </Box>
          ))}
        </Stack>
...
const BucketView = ({ bucket, refresh, setRefresh }) => {
  const {
    contextBuckets,
    currentBucket,
  ...
  // Example of missingFeatures object:
  // missingFeatures = {
  //   "User Features": [
  ...
  useEffect(() => {
    const calculatedMissingFeatures = Object.entries(
      contextBuckets[bucket].featureGroups
    )
      .filter(([_, featureGroup]) =>
        featureGroup.required_features.some((feature) =>
          isFeatureMissing(feature)
        )
      )
      .reduce((acc, [_, featureGroup]) => {
        acc[featureGroup.title] = featureGroup.required_features.filter(
          (feature) => isFeatureMissing(feature)
        );
        return acc;
      }, {});
    console.log("calculatedMissingFeatures", calculatedMissingFeatures);
    setMissingFeatures(calculatedMissingFeatures);
  }, [contextConfig]);
    ...
    <Box>
      ...
      <Grid container spacing={2} padding={"12px 16px 12px 16px"}>
        {Object.keys(contextBuckets[bucket].featureGroups)
          .filter(
            (featureGroup) =>
              contextBuckets[bucket].featureGroups[featureGroup].is_enabled
          )
          .map((featureGroup) => (
            <Grid
              item
              xs={12}
              md={
                Object.keys(contextBuckets[bucket].featureGroups).filter(
                  (featureGroup) =>
      ...
      {featureGroupOpen && currentFeatureGroup !== "None" && (
        <FeatureGroupDialog
          ...
          missingFeatures={
            missingFeatures[
              contextBuckets[bucket].featureGroups[currentFeatureGroup].title
            ] || []
          }
        />
      )}
