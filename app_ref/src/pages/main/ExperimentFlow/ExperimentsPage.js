...
import { listBuckets } from "../../../utils/s3Utils";
...
const ExperimentsPage = () => {
  console.log("ExperimentsPage loaded");
  ...
  const filteredExpList = experiments_list.filter((row) => !row.inTrash);
        ...
        <Box sx={{ border: "1px solid #EAECF0", borderRadius: "12px" }}>
          ...
          <TabContext value={tableTabValue}>
            ...
            <TabPanel value="liveTable" sx={{ padding: "16px 0" }}>
              <ExpTable currentFlow={"experiments"} currentTable={"live"} />
            </TabPanel>
            <TabPanel value="archiveTable" sx={{ padding: "16px 0" }}>
              <ExpTable currentFlow={"experiments"} currentTable={"archive"} />
            </TabPanel>
          </TabContext>
        </Box>
