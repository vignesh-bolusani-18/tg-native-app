...
const NewFilledBoxComponent = ({
  ...
  const { getDataByArrayName, removeItem, configState } = useConfig();
  ...
  const renderContent = () => {
    ...
    switch (arrayName) {
            ...
            {data.map((feature, index) => (
              ...
              >
                <Chip
                  ...
                  onDelete={() => removeItem(index, arrayName, featureGroup)}
                />
              </CustomTooltip>
            ))}
          </Stack>
                ...
                .map((adjustment, index) => (
                  ...
                  >
                    <Chip
                      ...
                      onDelete={() =>
                        removeItem(index + 1, arrayName, featureGroup)
                      }
                    />
                  </CustomTooltip>
                ))}
          ...
          <CustomScrollbar padding="2px">
            <Stack direction="row" spacing={2} sx={{ overflowX: "auto" }}>
              {data.map((reference, index) => {
                ...
                const experimentId = getExperimentIdFromPath(
                  reference.exp_path
                );
                ...
                return (
                  <CustomTooltip
                    ...
                    title={
                      <Stack spacing={0.5}>
                        <Typography>Experiment: {experimentName}</Typography>
                        <Typography>ID: {experimentId}</Typography>
                        <Typography>
                          Forecast Type: {reference.forecast_type}
                        </Typography>
                        <Typography>
                          Column Tag: {reference.column_tag}
                        </Typography>
                      </Stack>
                    }
                  >
                    <Chip
                      label={`${experimentName} - ${reference.forecast_type}`}
                      onDelete={() =>
                        removeItem(index, arrayName, featureGroup)
                      }
                    />
                  </CustomTooltip>
                );
      ...
      case "experiments_history":
        return (
          <CustomScrollbar padding="2px">
            <Stack direction="row" spacing={2} sx={{ overflowX: "auto" }}>
              {data.map((history, index) => {
                console.log("History Item", history);
                // Extract experiment ID the same as forecast_data_reference logic
                const getExperimentIdFromPath = (expPath) => {
                  const pathParts = expPath.split("/");
                  return pathParts[pathParts.length - 1];
                };

                // Get experiment name by ID using available experiments_list
                const getExperimentNameById = (experimentId) => {
                  const experiment = experiments_list.find(
                    (exp) => exp.experimentID === experimentId
                  );
                ...
                // If history is an object, take exp_path from history; if string, use history directly
                const expPath =
                ...
                const experimentId = getExperimentIdFromPath(expPath);
                const experimentName = getExperimentNameById(experimentId);

                // Format the experiment ID as (first3...last3)
                const formattedId =
                  experimentId.length > 6
                    ? `${experimentId.substring(
                        0,
                        3
                      )}...${experimentId.substring(experimentId.length - 3)}`
                    : experimentId;
                    ...
                    title={
                      <Stack spacing={0.5}>
                        ...
                        {history && typeof history === "object" && (
                          <Typography>Date Tag: {history.date_tag}</Typography>
                        )}
                      </Stack>
                    }
                  >
                    <Chip
                      label={`${experimentName} (${formattedId})`}
                      onDelete={() =>
                        removeItem(index, arrayName, featureGroup)
                      }
                      // If you want to add a delete option, you can add onDelete here
                    />
                  </CustomTooltip>
                );
              })}
...
const BucketView = ({ bucket, refresh, onRestore, setRefresh }) => {
  ...
  console.log(
    "isHyperParam " +
      configState?.training?.groups?.cluster?.model_names +
      " " +
      configState?.training?.groups?.cluster?.model_names.includes(
