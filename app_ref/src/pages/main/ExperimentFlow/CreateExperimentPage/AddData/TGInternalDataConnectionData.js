...
export default function TGInternalDataConnectionData({
  open,
  handleClose,
  ...
  React.useEffect(() => {
    ...
    if (currentDatasetTag !== "none") {
      ...
      console.log(combinedDatasets.map((dataset) => dataset.tag))
      setallDatasetTags(combinedDatasets.map((dataset) => dataset.tag));
    } else {
      setTags([]);
      console.log(AllDatasetTags)
      setallDatasetTags(AllDatasetTags);
    }
  }, [currentDatasetTag   ]);

  console.log("allDatasetTags", allDatasetTags);
  ...
  const handleConfirm = async () => {
    ...
    const apiArgPath = `accounts/${currentCompany.companyName}_${currentCompany.companyID}/customer_data/data_library/internal_data/${filename}.json`;
    const uploadCSVPath = `accounts/${currentCompany.companyName}_${currentCompany.companyID}/customer_data/data_library/internal_data/samples/${filename}.csv`;
    const apiReqArg = dataConnectionPayload;
    await uploadMetadataToS3({ metaData: metadata, path: metaDataPath });
    await uploadMetadataToS3({ metaData: apiReqArg, path: apiArgPath });
