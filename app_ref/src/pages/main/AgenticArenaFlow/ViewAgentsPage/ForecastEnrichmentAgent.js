...
import { setCurrentRLAgentEnrichmentRisk } from "../../../../redux/slices/dashboardSlice";
...
const extractKeys = (data, dateColumn) => {
  if (!data || typeof data !== 'object' || !dateColumn) {
    console.log("Invalid data or dateColumn", { data, dateColumn });
    return [];
  }
  ...
  if (keysToExtract.length === 0) {
    console.log("No keys to extract");
    return [];
  }

  const length = data[keysToExtract[0]]?.length || 0;
  if (length === 0) {
    ...
    (item) => {
      ...
      if (
        ...
        transformedItem = {
          ...
          P10: item.split === "future" ? item.P10 : undefined,
          P20: item.split === "future" ? item.P20 : undefined,
          P30: item.split === "future" ? item.P30 : undefined,
          P40: item.split === "future" ? item.P40 : undefined,
          P50: item.split === "future" ? item.P50 : undefined,
          P60: item.split === "future" ? item.P60 : undefined,
          P70: item.split === "future" ? item.P70 : undefined,
          P80: item.split === "future" ? item.P80 : undefined,
          P90: item.split === "future" ? item.P90 : undefined,
          P99: item.split === "future" ? item.P99 : undefined,
          offset_forecast:
...
const ForecastEnrichmentAgent = () => {
  ...
  console.log(dimensionFilterData);
  ...
  const { enrichment_bydate , configState } = useConfig();
  ...
  useEffect(() => {
  if (!demandForecastingData || !dateColumn || !demandForecastingData[dateColumn]) {
    return;
  }

  
  
  console.log(demandForecastingData, dateColumn);
...
}, [demandForecastingData, dateColumn]);


useEffect(() => {
  if (dimensionFilterData && Object.keys(dimensionFilterData).length > 0) {
    const preferredDimension = dimensionFilterData.hasOwnProperty("Forecast_Granularity") 
      ? "Forecast_Granularity" 
      : Object.keys(dimensionFilterData)[0];
    setCurrentDimension(preferredDimension);
  }
}, [dimensionFilterData]);
  /*   useEffect(() => {
  ...
  useEffect(() => {
    console.log(
      "Check Current Value",
      dimensionFilterData[currentDimension].includes(currentValue)
    );
    console.log("Current Value", currentValue);
    ...
    const CurrentValue = dimensionFilterData[currentDimension].includes(
      ...
      ? rlAgentEnrichmentSuggestions[0].Dimension === currentDimension
        ? rlAgentEnrichmentSuggestions[0].Value
        : dimensionFilterData[currentDimension][0]
      : dimensionFilterData[currentDimension][0];
       console.log("CurrentValue on both dimension and value change", CurrentValue);
  ...
  useEffect(() => {
    ...
    if(rlAgentEnrichmentSuggestions[0]){
      const CurrentValue =
      rlAgentEnrichmentSuggestions.length > 0
        ? rlAgentEnrichmentSuggestions[0].Dimension === currentDimension
          ? rlAgentEnrichmentSuggestions[0]?.Value
          : dimensionFilterData[currentDimension][0]
        : dimensionFilterData[currentDimension][0];
    console.log("CurrentValue on only dimension change", CurrentValue);
    setCurrentValue(CurrentValue);
    }
    
  }, [currentDimension ]);
  /*   useEffect(() => {
    setvalues(dimensionFilterData[currentRLAgentEnrichmentDimension]);
    setCurrentRLAgentEnrichmentValue(
      dimensionFilterData[currentRLAgentEnrichmentDimension][0]
    );
  ...
  const handleCurrentValueChange = (value) => {
    const index = rlAgentEnrichmentSuggestions.findIndex(
    ...
    if (index !== -1) {
      const newRLAgentEnrichmentSuggestion =
        rlAgentEnrichmentSuggestions[index];
      if (
        currentRLAgentEnrichmentSuggestion?.Value !==
        newRLAgentEnrichmentSuggestion?.Value
      ) {
        setCurrentRLAgentEnrichmentSuggestion(newRLAgentEnrichmentSuggestion);
        if (
          newRLAgentEnrichmentSuggestion.Risk !== currentRLAgentEnrichmentRisk
        ) {
          setCurrentRLAgentEnrichmentRisk(newRLAgentEnrichmentSuggestion.Risk);
        }
      }
    }
  };
  ...
  useEffect(() => {
    handleCurrentValueChange(currentValue);
  }, [currentValue]);
  ...
  useEffect(() => {
    /* const CurrentValue = dimensionFilterData[currentDimension].includes(
      ...
      : dimensionFilterData[currentDimension][0];
    setCurrentValue(CurrentValue); */
    const fetchRLAgentEnrichmentSuggestions = async () => {
      const dict = { Forecast_Granularity: "ts_id", Cluster: "cluster" };
      const changable = ["Forecast_Granularity", "Cluster"];
      const dimension = changable.includes(currentDimension)
        ? dict[currentDimension]
        : currentDimension;
      const approvedRLAgentEnrichmentSuggestions =
        await setApprovedRLAgentEnrichmentSuggestions(
          `${experimentBasePath}/rl_agent/approved_enrichments.csv`,
          {
            dimensionFilters: {},
            columnFilter: [],
            selectAllColumns: true,
      ...
      await setRLAgentEnrichmentSuggestions(
        ...
        1,
        configState?.stacking?.rl_agents
      );
    };
    fetchRLAgentEnrichmentSuggestions();
  }, [currentDimension, currentRLAgentEnrichmentRisk , rlAgentEnrichmentSuggestions?.length]);
  ...
  const handleClearFilter = async () => {
    ...
    //setCurrentRLAgentEnrichmentDimension("all");
    setCurrentStartDate(demandForecastingData[dateColumn][0]);
    const length = demandForecastingData[dateColumn].length;
    setCurrentEndDate(demandForecastingData[dateColumn][length - 1]);
  };
  /*   useEffect(() => {
    setCurrentDimension(currentRLAgentEnrichmentDimension);
    setCurrentValue(currentRLAgentEnrichmentValue);
  }, [currentRLAgentEnrichmentDimension, currentRLAgentEnrichmentValue]); */
  ...
  const dimensions = data && data.length > 0 ? Object.keys(data[0])
  ...
  return (
    <Stack sx={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
      ...
      >
        ...
        />
        <CustomAutocomplete
  key=  {`value-autocomplete-${values.length}`}
  disableClearable
  showLabel
  label="Value"
  placeholder="select option.."
...
/>
      ...
      <Box sx={{ height: "100%" }}>
        ...
        <Grid
          ...
          paddingY={"16px"}
          // border={"1px solid"}
          sx={{ height: "600px" }} // Fixed height for both containers
        >
          <Grid
            item
            xs={12}
            ...
            sx={{
              ...
              height: "100%", // Match height with second Grid item
            }}
          >
          ...
          >
            ...
            >
              <Stack direction={"row"} spacing={"0.5rem"}>
                ...
                />
                {data && data.length > 0 && data[0] && "actual_raw" in data[0] && (
                  <FormControlLabel
                    sx={{
                      whiteSpace: "nowrap", // Prevents the label from wrapping
                      flexShrink: 0, // Prevents it from shrinking or moving to a new line
                ...
                <FormControlLabel
                  ...
                  label="ML Forecast"
                />
                {data && data.length > 0 && data[0] && "offset_forecast" in data[0] && (
                  <FormControlLabel
                    sx={{
                      whiteSpace: "nowrap", // Prevents the label from wrapping
                      flexShrink: 0, // Prevents it from shrinking or moving to a new line
            ...
            {selectedDimensions.map((dimension, index) => {
              ...
              if (dimension === "pred") {
                ...
                const rlSuggestionsInfo = currentRLAgentEnrichmentKeys.map(
                  (suggestion) =>
                    dimensions.find((dim) => dim.label === suggestion)
                );
                    ...
                    <Box sx={{ marginBottom: "20px" }} key="pred-actual">
                      ...
                      >
                        ...
                        >
                          ...
                          {rlSuggestionsInfo.map((suggestion, index) => (
                            <Area
                              type="linear"
                              dataKey={suggestion.key}
                              strokeWidth={2}
...
export default ForecastEnrichmentAgent;
