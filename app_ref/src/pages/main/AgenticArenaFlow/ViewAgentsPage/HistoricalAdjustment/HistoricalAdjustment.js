        ...
        if (
          ...
          switch (enrichmentType) {
            case "uplift":
              ...
              break;
            case "upper_bound":
              cumulativeForecast = item.pred_upper_bound; // Update the forecast with the upper bound
              break;
            case "lower_bound":
...
function applyAdjustment(
  ...
  return transformedData.map((item) => {
    let cumulativeAdjust;

    cumulativeAdjust = item.actual;
    // Start with the initial forecast
    adjustmentOperations = adjustmentOperations.filter(
      (adjust) => adjust.operation === "adjust_data"
    );
    adjustmentOperations.forEach((adjust) => {
      const adjustmentType = adjust.kwargs.adjustment_type;
      const startDate = new Date(adjust.kwargs.date_range[0]);
      const endDate = new Date(adjust.kwargs.date_range[1]);
      const itemDate = new Date(item[dateColumn]);
      const timeSteps = adjust.kwargs?.time_steps;
      const referenceData = adjust.kwargs?.future_history;
      ...
      if (itemDate >= startDate && itemDate <= endDate) {
        if (
          adjust.kwargs.dimension === "None" ||
          (adjust.kwargs.dimension === currentDimension &&
            adjust.kwargs.value === currentValue)
        ) {
          // Apply the uplift recursively
          const upliftAmount =
            (cumulativeAdjust * adjust.kwargs.adjustment_value) / 100;
          switch (adjustmentType) {
            case "uplift":
              cumulativeAdjust += upliftAmount; // Update the forecast with the uplift
              break;
    ...
    console.log("cumulative " + cumulativeAdjust + item.actual);
    item.adjusted_historical_data = cumulativeAdjust;

    return item;
  });
}

async function applyAdjustmentParallel(
  transformedData,
  adjustmentOperations,
  currentDimension,
  currentValue,
...
const get_graph_data = async (
  ...
  // const enriched_data = applyEnrichment(
  //   transformedData1,
  //   enrichment_bydate,
  //   currentDimension,
  //   currentValue,
  ...
  const adjusted_data = await applyAdjustmentParallel(
    transformedData1,
    operations,
  ...
  console.log("adjusted Data", adjusted_data);
...
const HistoricalAdjustment = () => {
  ...
  const handleToggleAdjustHistoricalData = () => {
    setShowAdjustHistoricalData(!showAdjustHistoricalData);
  };
        ...
        >
          <Grid
            ...
            sx={{
              ...
              height: "80%", // Match height with second Grid item
            }}
          >
            <AdjustmentBox
            ...
            >
              <Stack direction={"row"} spacing={"0.5rem"}>
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      checked={showAdjustHistoricalData}
                      onChange={handleToggleAdjustHistoricalData}
                      color="primary"
                      sx={{
                        transform: "scale(0.8)",
                      }}
                      ...
                      >
                        ...
                        >
                          ...
                          {showAdjustHistoricalData && (
                            <Area
                              type="linear"
                              dataKey={"adjusted_historical_data"}
                              strokeWidth={2}
                              stroke={"purple"}
                              fill="none"
                              dot={{
...
export default HistoricalAdjustment;
